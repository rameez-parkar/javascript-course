...
console.log(`Sarah's dog is eating ${eatingHabit(sarahDog)}`);

// 3.
const ownersEatTooMuch = dogs.filter(dog => dog.curFood > 1.1 * dog.recommendedFood).flatMap((dog) => dog.owners);
const ownersEatTooLittle = dogs.filter(dog => dog.curFood < 0.9 * dog.recommendedFood).flatMap((dog) => dog.owners);
...
const dogsEatingOkayFood = dogs.filter(dog => {
  return dog.curFood >= 0.9*dog.recommendedFood && dog.curFood <= 1.1*dog.recommendedFood;
});
